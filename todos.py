...
OUTPUT_DIR = Path("outjson")   # carpeta para el JSON único
...
for path in images:
    ...
    if g is None:
        ...
        resultados.append({"image": path.name, "ok": False, "msg": "no se pudo leer"})
        continue
    ...
    if not contours:
        resultados.append({"image": path.name, "ok": False, "msg": "sin contornos"})
        continue
...
OUT_FILE = OUTPUT_DIR / "todos_los_objetos.json"
with open(OUT_FILE, "w", encoding="utf-8") as f:
    json.dump(resultados, f, indent=2, ensure_ascii=False)

print("✅ Listo. JSON único escrito en:", OUT_FILE)

...
def process_images(input_dir="img2", output_dir="out"):
    """
    Procesa imágenes .jpg aplicando filtrado, umbral adaptativo y segmentación
    para obtener máscaras binarias refinadas.

    Args:
        input_dir (str | Path): Carpeta de entrada con imágenes JPG.
        output_dir (str | Path): Carpeta de salida para las imágenes procesadas.
    """
    ...
    for path in sorted(input_dir.glob("*.jpg")):
        ...
        if img is None:
            print(f"⚠️ No se pudo leer {path.name}")
            continue
        ...
        if not cnts:
            print("⚠️ No se hallaron contornos. Se guarda edges como referencia.")
